...
description: "In order to instantiate a class template, every template argument must be known, but not every template argument has to be specified."
source_url: "https://en.cppreference.com/w/cpp/language/class_template_argument_deduction"
language: "C++"
---

{/* source:cpp-language-class-template-argument-deduction:0000 */}
In order to instantiate a [class template](/docs/cpp/language/templates/class_template), every template argument must be known, but not every template argument has to be specified. In the following contexts the compiler will deduce the template arguments from the type of the initializer:

{/* source:cpp-language-class-template-argument-deduction:0001 */}
- any [declaration](/docs/cpp/language/declarations) that specifies initialization of a variable and variable template, whose declared type is the class template (possibly [cv-qualified](/docs/cpp/language/declarations/cv)):

{/* source:cpp-language-class-template-argument-deduction:0002 */}
```cpp
...
{/* source:cpp-language-class-template-argument-deduction:0003 */}
- [new-expressions](/docs/cpp/language/expressions/new):

{/* source:cpp-language-class-template-argument-deduction:0004 */}
```cpp
template<class T>
struct A
{
...
{/* source:cpp-language-class-template-argument-deduction:0005 */}
- [function-style cast](/docs/cpp/language/expressions/explicit_cast) expressions:

{/* source:cpp-language-class-template-argument-deduction:0006 */}
```cpp
...
{/* source:cpp-language-class-template-argument-deduction:0007 */}
<Revision since="C++20">
- the type of a [non-type template parameter](/docs/cpp/language/templates/template_parameters#Non-type_template_parameter):
...
{/* source:cpp-language-class-template-argument-deduction:0008 */}
...
{/* source:cpp-language-class-template-argument-deduction:0009 */}
...
{/* source:cpp-language-class-template-argument-deduction:0010 */}
...
{/* source:cpp-language-class-template-argument-deduction:0011 */}
<Revision since="C++20">
  - The [associated constraints](/docs/cpp/language/templates/constraints) of `Fi` are the conjunction of the associated constraints of `C` and the associated constraints of `Ci`.
</Revision>

{/* source:cpp-language-class-template-argument-deduction:0012 */}
  - The [parameter list](/docs/cpp/language/functions/function#Parameter_list) of `Fi` is the parameter list of `Ci`.
  - The return type of `Fi` is `C` followed by the template parameters of the class template enclosed in `<>`.

{/* source:cpp-language-class-template-argument-deduction:0013 */}
...
{/* source:cpp-language-class-template-argument-deduction:0014 */}
...
{/* source:cpp-language-class-template-argument-deduction:0015 */}
- For each [user-defined deduction guide](/docs/cpp/language/templates/ctad#User-defined_deduction_guides) `Gi`, a fictional function or function template `Fi`, is constructed, such that all following conditions are satisfied:

{/* source:cpp-language-class-template-argument-deduction:0016 */}
...
{/* source:cpp-language-class-template-argument-deduction:0017 */}
<Revision since="C++20">
- In addition, if
  - `C` is defined and satisfies the requirements of an [aggregate type](/docs/cpp/language/initialization/aggregate_initialization) with the assumption that any dependent base class has no virtual functions or virtual base classes,
  - there are no user-defined deduction guides for `C`, and
  - the variable is initialized from a non-empty list of initializers `arg1, arg2, ..., argn` (which may use [designated initializer](/docs/cpp/language/initialization/aggregate_initialization#Designated_initializer)),
  ...
  - Let `ei` be the (possibly recursive) [aggregate element](/docs/cpp/language/initialization/aggregate_initialization) that would be initialized from `argi`, where
    - [brace elision](/docs/cpp/language/initialization/aggregate_initialization#Brace_elision) is not considered for any aggregate element that has
      - a [dependent](/docs/cpp/language/templates/dependent_name#Dependent_types) non-array type,
      - an array type with a [value-dependent](/docs/cpp/language/templates/dependent_name#Value-dependent_expressions) bound, or
      - an array type with a dependent array element type and `argi` is a [string literal](/docs/cpp/language/expressions/string_literal)
    - if `C` (or its element that is itself an aggregate) has a base that is a [pack expansion](/docs/cpp/language/templates/parameter_pack):
  ...
  - If there is no such `ei`, the aggregate deduction candidate is not added. Otherwise, determine the parameter list `T1, T2, ..., Tn` of the aggregate deduction candidate as follows:
    - If `ei` is an array and `argi` is a braced-init-list, `Ti` is an rvalue reference to the declared type of `ei`.
    - If `ei` is an array and `argi` is a [string literal](/docs/cpp/language/expressions/string_literal), `Ti` is an lvalue reference to the const-qualified declared type of `ei`.
...
{/* source:cpp-language-class-template-argument-deduction:0018 */}
[Template argument deduction](/docs/cpp/language/templates/template_argument_deduction) and [overload resolution](/docs/cpp/language/functions/overload_resolution) is then performed for initialization of a fictional object of hypothetical class type, whose constructor signatures match the guides (except for return type) for the purpose of forming an overload set, and the initializer is provided by the context in which class template argument deduction was performed, except that the first phase of [list-initialization](/docs/cpp/language/functions/overload_resolution#List-initialization) (considering initializer-list constructors) is omitted if the initializer list consists of a single expression of type (possibly cv-qualified) `U`, where `U` is a specialization of `C` or a class derived from a specialization of `C`.

{/* source:cpp-language-class-template-argument-deduction:0019 */}
...
{/* source:cpp-language-class-template-argument-deduction:0020 */}
```cpp
template<class T>
struct UniquePtr
{
...
{/* source:cpp-language-class-template-argument-deduction:0021 */}
...
{/* source:cpp-language-class-template-argument-deduction:0022 */}
```cpp
template<class T>
struct S
{
...
{/* source:cpp-language-class-template-argument-deduction:0023 */}
...
{/* source:cpp-language-class-template-argument-deduction:0024 */}
<DeclarationDoc>
  <Declaration id="1" language="cpp" grammar code={`explicit (optional) template-name ( parameter-list ) -> simple-template-id requires-clause (optional) ;`} />
  <Declaration id="2" language="cpp" grammar code={`template < template-parameter-list > requires-clause (optional)
explicit (optional) template-name ( parameter-list ) -> simple-template-id requires-clause (optional) ;`} />
</DeclarationDoc>

{/* source:cpp-language-class-template-argument-deduction:0025 */}
<ParameterList>
  <Parameter name="template-parameter-list">a non-empty comma-separated list of [template parameters](/docs/cpp/language/templates/template_parameters)</Parameter>
  <Parameter name="explicit">an [`explicit` specifier](/docs/cpp/language/classes/explicit)</Parameter>
  <Parameter name="template-name">the name of the class template whose arguments are to be deduced</Parameter>
  <Parameter name="parameter-list">a (possibly empty) [parameter list](/docs/cpp/language/functions/function#Parameter_list)</Parameter>
  <Parameter name="simple-template-id">a [simple template identifier](/docs/cpp/language/templates#Template_identifiers)</Parameter>
  <Parameter name="requires-clause"><InlineRevision since="C++20" /> a [`requires` clause](/docs/cpp/language/templates/constraints#Requires_clauses)</Parameter>
</ParameterList>

{/* source:cpp-language-class-template-argument-deduction:0026 */}
<Revision since="C++20">
The parameters of user-defined deduction guides cannot have placeholder types: the [abbreviated function template](/docs/cpp/language/templates/function_template#Abbreviated_function_template) syntax is not allowed.
</Revision>

{/* source:cpp-language-class-template-argument-deduction:0027 */}
...
{/* source:cpp-language-class-template-argument-deduction:0028 */}
A deduction guide is not a function and does not have a body. Deduction guides are not found by name lookup and do not participate in overload resolution except for the [overload resolution against other deduction guides](/docs/cpp/language/functions/overload_resolution#Best_viable_function) when deducing class template arguments. Deduction guides cannot be redeclared in the same translation unit for the same class template.

{/* source:cpp-language-class-template-argument-deduction:0029 */}
```cpp
...
{/* source:cpp-language-class-template-argument-deduction:0030 */}
The fictional constructors for the purpose of overload resolution (described above) are explicit if they correspond to an implicitly-generated deduction guide formed from an explicit constructor or to a user-defined deduction guide that is declared [explicit](/docs/cpp/language/classes/explicit). As always, such constructors are ignored in copy-initialization context:

{/* source:cpp-language-class-template-argument-deduction:0031 */}
```cpp
template<class T>
struct A
{
...
{/* source:cpp-language-class-template-argument-deduction:0032 */}
...
{/* source:cpp-language-class-template-argument-deduction:0033 */}
```cpp
template<class T>
struct B
{
...
{/* source:cpp-language-class-template-argument-deduction:0034 */}
<Revision since="C++20">
...
- Substitute the result of above deduction into `f`, if substitution fails, no guide is produced; otherwise, let `g` denote the result of substitution, a guide `f'` is formed, such that
  ...
  - The associated [constraints](/docs/cpp/language/templates/constraints) of `f'` are the conjunction of the associated constraints of `g` and a constraint that is satisfied if and only if the arguments of `A` are deducible from the result type
...
{/* source:cpp-language-class-template-argument-deduction:0035 */}
...
{/* source:cpp-language-class-template-argument-deduction:0036 */}
```cpp
...
{/* source:cpp-language-class-template-argument-deduction:0037 */}
<Revision until="C++20">
...
{/* source:cpp-language-class-template-argument-deduction:0038 */}
User-defined deduction guides do not have to be templates:

{/* source:cpp-language-class-template-argument-deduction:0039 */}
```cpp
template<class T>
struct S
{
...
{/* source:cpp-language-class-template-argument-deduction:0040 */}
...
{/* source:cpp-language-class-template-argument-deduction:0041 */}
```cpp
template<class T>
struct X
{
...
{/* source:cpp-language-class-template-argument-deduction:0042 */}
In [overload resolution](/docs/cpp/language/functions/overload_resolution#Best_viable_function), partial ordering takes precedence over whether a function template is generated from a user-defined deduction guide: if the function template generated from the constructor is more specialized than the one generated from the user-defined deduction guide, the one generated from the constructor is chosen. Because the copy deduction candidate is typically more specialized than a wrapping constructor, this rule means that copying is generally preferred over wrapping.

{/* source:cpp-language-class-template-argument-deduction:0043 */}
```cpp
template<class T>
struct A
{
...
{/* source:cpp-language-class-template-argument-deduction:0044 */}
...
{/* source:cpp-language-class-template-argument-deduction:0045 */}
...
{/* source:cpp-language-class-template-argument-deduction:0046 */}
```cpp
template<class T>
struct A
{
...
{/* source:cpp-language-class-template-argument-deduction:0047 */}
An rvalue reference to a cv-unqualified template parameter is not a [forwarding reference](/docs/cpp/language/templates/template_argument_deduction) if that parameter is a class template parameter:

{/* source:cpp-language-class-template-argument-deduction:0048 */}
```cpp
template<class T>
struct A
{
...
{/* source:cpp-language-class-template-argument-deduction:0049 */}
...
{/* source:cpp-language-class-template-argument-deduction:0050 */}
```cpp
...
{/* source:cpp-language-class-template-argument-deduction:0051 */}
...
{/* source:cpp-language-class-template-argument-deduction:0052 */}
```cpp
std::vector v1{1, 2}; // std::vector<int>
...
{/* source:cpp-language-class-template-argument-deduction:0053 */}
...
{/* source:cpp-language-class-template-argument-deduction:0054 */}
```cpp
std::tuple p1{1, 1.0};             //std::tuple<int, double>, using deduction
auto p2 = std::make_tuple(1, 1.0); //std::tuple<int, double>, pre-C++17
```

{/* source:cpp-language-class-template-argument-deduction:0055 */}
| Feature-test macro | Value | Std | Feature |
| --- | --- | --- | --- |
| [`__cpp_deduction_guides`](/docs/cpp/library/feature_test#cpp_deduction_guides) | [`201703L`](/docs/cpp/library/compiler_support/17#cpp_deduction_guides_201703L) | (C++17) | Template argument deduction for class templates |
| | [`201907L`](/docs/cpp/library/compiler_support/20#cpp_deduction_guides_201907L) | (C++20) | CTAD for aggregates and aliases |

## Defect reports

{/* source:cpp-language-class-template-argument-deduction:0056 */}
The following behavior-changing defect reports were applied retroactively to previously published C++ standards.

{/* source:cpp-language-class-template-argument-deduction:0057 */}
<DefectReportList>
  <DefectReport kind="cwg" id={2376} standard="C++17"><PublishedBehavior>CTAD would be performed even if the type of the variable declared is different from the class template whose arguments will be deduced</PublishedBehavior><CorrectedBehavior>do not perform CTAD in this case</CorrectedBehavior></DefectReport>
  <DefectReport kind="cwg" id={2628} standard="C++20"><PublishedBehavior>implicit deduction guides did not propagate constraints</PublishedBehavior><CorrectedBehavior>propogate constraints</CorrectedBehavior></DefectReport>
  <DefectReport kind="cwg" id={2697} standard="C++20"><PublishedBehavior>it was unclear whether the abbreviated function template syntax is allowed in user-defined deduction guides</PublishedBehavior><CorrectedBehavior>prohibited</CorrectedBehavior></DefectReport>
  <DefectReport kind="cwg" id={2707} standard="C++20"><PublishedBehavior>deduction guides could not have a trailing `requires` clause</PublishedBehavior><CorrectedBehavior>they can</CorrectedBehavior></DefectReport>
  <DefectReport kind="cwg" id={2714} standard="C++17"><PublishedBehavior>implicit deduction guides did not consider the default aguments of constructors</PublishedBehavior><CorrectedBehavior>consider them</CorrectedBehavior></DefectReport>
  <DefectReport kind="cwg" id={2913} standard="C++20"><PublishedBehavior>the resolution of [CWG issue 2707](https://cplusplus.github.io/CWG/issues/2707.html) made the deduction guide syntax inconsistent with the function declaration syntax</PublishedBehavior><CorrectedBehavior>adjusted the syntax</CorrectedBehavior></DefectReport>
  <DefectReport kind="paper" id="P0702R1" standard="C++17"><PublishedBehavior>an initializer-list constructor can pre-empt the copy deduction candidate, resulting in wrapping</PublishedBehavior><CorrectedBehavior>initializer-list phase skipped when copying</CorrectedBehavior></DefectReport>
</DefectReportList>
