...
- the *promise object*, manipulated from inside the coroutine. The coroutine submits its result or exception through this object. Promise objects are in no way related to [`std::promise`](/docs/cpp/library/thread/promise).
