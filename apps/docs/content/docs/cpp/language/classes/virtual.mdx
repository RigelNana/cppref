...
description: "The virtual specifier specifies that a non-static member function is virtual and supports dynamic dispatch."
source_url: "https://en.cppreference.com/w/cpp/language/virtual"
language: "C++"
---

{/* source:cpp-language-virtual:0000 */}
The `virtual` specifier specifies that a non-static [member function](/docs/cpp/language/classes/member_functions) is virtual and supports dynamic dispatch. It may only appear in the *decl-specifier-seq* of the initial declaration of a non-static member function (i.e., when it is declared in the class definition).

## Explanation

{/* source:cpp-language-virtual:0001 */}
Virtual functions are member functions whose behavior can be overridden in derived classes. As opposed to non-virtual functions, the overriding behavior is preserved even if there is no compile-time information about the actual type of the class. That is to say, if a derived class is handled using pointer or reference to the base class, a call to an overridden virtual function would invoke the behavior defined in the derived class. Such a function call is known as virtual function call or virtual call. Virtual function call is suppressed if the function is selected using [qualified name lookup](/docs/cpp/language/basic_concepts/lookup) (that is, if the function's name appears to the right of the scope resolution operator `::`).

{/* source:cpp-language-virtual:0002 */}
```cpp
#include <iostream>
...
{/* source:cpp-language-virtual:0003 */}
If some member function `vf` is declared as virtual in a class `Base`, and some class `Derived`, which is derived, directly or indirectly, from `Base`, has a declaration for member function with the same

{/* source:cpp-language-virtual:0004 */}
- name
- parameter type list (but not the return type)
- cv-qualifiers
- ref-qualifiers

{/* source:cpp-language-virtual:0005 */}
Then this function in the class `Derived` is also virtual (whether or not the keyword `virtual` is used in its declaration) and overrides `Base::vf` (whether or not the word `override` is used in its declaration).

{/* source:cpp-language-virtual:0006 */}
...
{/* source:cpp-language-virtual:0007 */}
```cpp
class B
{
...
{/* source:cpp-language-virtual:0008 */}
...
{/* source:cpp-language-virtual:0009 */}
```cpp
...
{/* source:cpp-language-virtual:0014 */}
<Revision since="C++11">
...
{/* source:cpp-language-virtual:0017 */}
<Revision since="C++20">
...
A [`consteval`](/docs/cpp/language/declarations/consteval) virtual function must not override or be overidden by a non-consteval virtual function.
</Revision>

{/* source:cpp-language-virtual:0018 */}
[Default arguments](/docs/cpp/language/functions/default_arguments) for virtual functions are substituted at the compile time.

### Covariant return types

{/* source:cpp-language-virtual:0019 */}
...
{/* source:cpp-language-virtual:0020 */}
...
- the return type of `Derived::f()` must be equally or less [cv-qualified](/docs/cpp/language/declarations/cv) than the return type of `Base::f()`.

{/* source:cpp-language-virtual:0021 */}
The class in the return type of `Derived::f` must be either `Derived` itself, or must be a [complete type](/docs/cpp/language/basic_concepts/incomplete_type) at the point of declaration of `Derived::f`.

{/* source:cpp-language-virtual:0022 */}
When a virtual function call is made, the type returned by the final overrider is [implicitly converted](/docs/cpp/language/expressions/implicit_cast) to the return type of the overridden function that was called:

{/* source:cpp-language-virtual:0023 */}
```cpp
class B {};
...
{/* source:cpp-language-virtual:0024 */}
...
{/* source:cpp-language-virtual:0025 */}
```cpp
class Base
{
public:
...
{/* source:cpp-language-virtual:0030 */}
```cpp
struct V
{
    virtual void f();
...
{/* source:cpp-language-virtual:0031 */}
[`virtual`](/docs/cpp/language/keywords/virtual)

## Defect reports

{/* source:cpp-language-virtual:0032 */}
The following behavior-changing defect reports were applied retroactively to previously published C++ standards.

{/* source:cpp-language-virtual:0033 */}
<DefectReportList>
  <DefectReport kind="cwg" id={258} standard="C++98">
    <PublishedBehavior>a non-const member function of a derived class might become virtual because of a const virtual member function of its base</PublishedBehavior>
    <CorrectedBehavior>virtuality also require cv-qualifications to be the same</CorrectedBehavior>
  </DefectReport>
  <DefectReport kind="cwg" id={477} standard="C++98">
    <PublishedBehavior>a friend declaration could contain the `virtual` specifier</PublishedBehavior>
    <CorrectedBehavior>not allowed</CorrectedBehavior>
  </DefectReport>
  <DefectReport kind="cwg" id={1516} standard="C++98">
    <PublishedBehavior>the definition of the terms "virtual function call" and "virtual call" were not provided</PublishedBehavior>
...
{/* source:cpp-language-virtual:0034 */}
<DescriptionList>
  <DescriptionItem>
    <DescriptionTerm>[derived classes and modes of inheritance](/docs/cpp/language/classes/derived_class)</DescriptionTerm>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>[`override` specifier](/docs/cpp/language/classes/override) <InlineRevision since="C++11" /></DescriptionTerm>
    <DescriptionBody>explicitly declares that a method overrides another method</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
