...
description: "Returns a std::stop_token associated with the same shared stop-state held internally by the jthread object."
source_url: "https://en.cppreference.com/w/cpp/thread/jthread/get_stop_token"
language: "C++"
---

{/* source:cpp-thread-jthread-get-stop-token:0000 */}
<DeclarationDoc>
  <Declaration language="cpp" since="C++20" code={`std::stop_token get_stop_token() const noexcept;`} />
</DeclarationDoc>

{/* source:cpp-thread-jthread-get-stop-token:0001 */}
Returns a [`std::stop_token`](/docs/cpp/library/thread/stop_token) associated with the same shared stop-state held internally by the `jthread` object.

### Parameters

{/* source:cpp-thread-jthread-get-stop-token:0002 */}
(none)

### Return value

{/* source:cpp-thread-jthread-get-stop-token:0003 */}
A value of type [`std::stop_token`](/docs/cpp/library/thread/stop_token) associated with stop-state held internally by `jthread` object.

### Example

{/* source:cpp-thread-jthread-get-stop-token:0004 */}
```cpp
#include <chrono>
