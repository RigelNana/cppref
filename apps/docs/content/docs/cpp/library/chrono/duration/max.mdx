...
description: "Returns a duration with the largest possible value."
source_url: "https://en.cppreference.com/w/cpp/chrono/duration/max"
language: "C++"
---

{/* source:cpp-chrono-duration-max:0000 */}
<DeclarationDoc>
  <Declaration language="cpp" until="C++20" code={`static constexpr duration max();`} />
  <Declaration language="cpp" since="C++20" code={`static constexpr duration max() noexcept;`} />
</DeclarationDoc>

{/* source:cpp-chrono-duration-max:0001 */}
Returns a duration with the largest possible value.

{/* source:cpp-chrono-duration-max:0002 */}
If the representation `rep` of the duration requires some other implementation to return a maximum-length duration, [`std::chrono::duration_values`](/docs/cpp/library/chrono/duration/../duration_values) can be specialized to return the desired value.

## Parameters

{/* source:cpp-chrono-duration-max:0003 */}
(none)

## Return value

{/* source:cpp-chrono-duration-max:0004 */}
`duration(std::chrono::duration_values<rep>::max())`

## Example

{/* source:cpp-chrono-duration-max:0005 */}
```cpp
#include <chrono>
#include <cstdint>
#include <iomanip>
...
{/* source:cpp-chrono-duration-max:0006 */}
<DescriptionList>
  <DescriptionItem>
  ...
  <DescriptionItem>
    ...
    <DescriptionBody>returns the special duration value min (public static member function)</DescriptionBody>
  </DescriptionItem>
</DescriptionList>
