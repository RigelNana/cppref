---
title: "std::wcsftime"
description: "Converts the date and time information from a given calendar time time to a null-terminated wide character string str according to format string format."
source_url: "https://en.cppreference.com/w/cpp/chrono/c/wcsftime"
language: "C++"
---

{/* source:cpp-chrono-c-wcsftime:0000 */}
<DeclarationDoc>
Defined in header [`<cwchar>`](/docs/cpp/library/headers/cwchar)

  <Declaration language="cpp" code={`std::size_t wcsftime( wchar_t* str, std::size_t count, const wchar_t* format, const std::tm* time );`} />
</DeclarationDoc>

{/* source:cpp-chrono-c-wcsftime:0001 */}
Converts the date and time information from a given calendar time `time` to a null-terminated wide character string `str` according to [format string](/docs/cpp/library/chrono/c/wcsftime#Format_string) `format`. Up to `count` wide characters are written.

## Parameters

{/* source:cpp-chrono-c-wcsftime:0002 */}
<ParameterList>
  <Parameter name="str">pointer to the first element of the `wchar_t` array for output</Parameter>
  <Parameter name="count">maximum number of wide characters to write</Parameter>
  <Parameter name="format">pointer to a null-terminated wide character string specifying the [format of conversion](/docs/cpp/library/chrono/c/wcsftime#Format_string)</Parameter>
  <Parameter name="time">pointer to the date and time information to be converted</Parameter>
</ParameterList>

## Format string

{/* source:cpp-chrono-c-wcsftime:0003 */}
The format string consists of zero or more conversion specifiers and ordinary characters (except `%`). All ordinary characters, including the terminating null character, are copied to the output string without modification. Each conversion specification begins with `%` character, optionally followed by `E` or `O` modifier (ignored if unsupported by the locale), followed by the character that determines the behavior of the specifier. The following format specifiers are available:

{/* source:cpp-chrono-c-wcsftime:0004 */}
...
{/* source:cpp-chrono-c-wcsftime:0005 */}
...
{/* source:cpp-chrono-c-wcsftime:0006 */}
```cpp
#include <ctime>
#include <cwchar>
#include <iostream>
...
{/* source:cpp-chrono-c-wcsftime:0007 */}
<DescriptionList>
  <DescriptionItem>
...
[C documentation](/docs/c/library/chrono/wcsftime) for wcsftime
