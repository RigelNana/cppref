...
description: "Registers a user-defined function which will be called by imbue(), std::basic_ios::copyfmt() and ~ios_base()."
source_url: "https://en.cppreference.com/w/cpp/io/ios_base/register_callback"
language: "C++"
---

{/* source:cpp-io-ios-base-register-callback:0000 */}
<DeclarationDoc>
  <Declaration language="cpp" code={`void register_callback( event_callback function, int index );`} />
</DeclarationDoc>

{/* source:cpp-io-ios-base-register-callback:0001 */}
Registers a user-defined function which will be called by [`imbue()`](/docs/cpp/library/io/ios_base/imbue), [`std::basic_ios::copyfmt()`](/docs/cpp/library/io/basic_ios/copyfmt) and `~ios_base()`. Every registered callback is called every time: the event type (a value of type [`event`](/docs/cpp/library/io/ios_base/event)) is passed as its first argument, and may be used to distinguish between the callers.

{/* source:cpp-io-ios-base-register-callback:0002 */}
...
{/* source:cpp-io-ios-base-register-callback:0003 */}
The user-defined callback function is not allowed to throw exceptions.

### Parameters

{/* source:cpp-io-ios-base-register-callback:0004 */}
<ParameterList>
  <Parameter name="function">the function which will be called on event, supplied as a function pointer of type [`event_callback`](/docs/cpp/library/io/ios_base/event_callback)</Parameter>
  <Parameter name="index">custom parameter which will be passed to the function</Parameter>
</ParameterList>

### Return value

{/* source:cpp-io-ios-base-register-callback:0005 */}
(none)

### Notes

{/* source:cpp-io-ios-base-register-callback:0006 */}
Once registered, a callback cannot be deregistered: it remains a part of the stream object for the rest of its lifetime. If the behavior of a callback needs to change, it may be controlled through [`iword()`](/docs/cpp/library/io/ios_base/iword) or [`pword()`](/docs/cpp/library/io/ios_base/pword).

{/* source:cpp-io-ios-base-register-callback:0007 */}
If the same function is registered multiple times, it is called multiple times.

{/* source:cpp-io-ios-base-register-callback:0008 */}
The integer value that is stored together with the callback is typically an index obtained from [`xalloc()`](/docs/cpp/library/io/ios_base/xalloc).

### Example

{/* source:cpp-io-ios-base-register-callback:0009 */}
