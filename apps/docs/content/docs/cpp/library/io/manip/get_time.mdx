---
title: "std::get_time"
description: "When used in an expression in >> get_time(tmb, fmt), parses the character input as a date/time value according to format string fmt according to the std::time_get facet of the locale currently imbued in the input stream in. The resultant value is stored in a std::tm object pointed to by tmb."
source_url: "https://en.cppreference.com/w/cpp/io/manip/get_time"
language: "C++"
---

{/* source:cpp-io-manip-get-time:0000 */}
<DeclarationDoc>
...
{/* source:cpp-io-manip-get-time:0001 */}
When used in an expression `in >> get_time(tmb, fmt)`, parses the character input as a date/time value according to format string `fmt` according to the [`std::time_get`](/docs/cpp/library/text/locale/time_get) facet of the locale currently imbued in the input stream `in`. The resultant value is stored in a [`std::tm`](/docs/cpp/library/chrono/c/tm) object pointed to by `tmb`.

### Parameters

{/* source:cpp-io-manip-get-time:0002 */}
<ParameterList>
  <Parameter name="tmb">valid pointer to the [`std::tm`](/docs/cpp/library/chrono/c/tm) object where the result will be stored</Parameter>
  <Parameter name="fmt">pointer to a null-terminated `CharT` string specifying the conversion format</Parameter>
</ParameterList>
...
{/* source:cpp-io-manip-get-time:0003 */}
An object of unspecified type such that

{/* source:cpp-io-manip-get-time:0004 */}
...
{/* source:cpp-io-manip-get-time:0005 */}
where the function `f` is defined as:

{/* source:cpp-io-manip-get-time:0006 */}
```cpp
...
{/* source:cpp-io-manip-get-time:0007 */}
As specified in [`std::time_get::do_get`](/docs/cpp/library/text/locale/time_get/get), which this function calls, it's unspecified if this function zero out the fields in `*tmb` that are not set directly by the conversion specifiers that appear in `fmt`: portable programs should initialize every field of `*tmb` to zero before calling `std::get_time`.

### Example

{/* source:cpp-io-manip-get-time:0008 */}
...
{/* source:cpp-io-manip-get-time:0009 */}
<DescriptionList>
  <DescriptionItem>
  ...
  <DescriptionItem>
    ...
    <DescriptionBody>parses a `chrono` object from a stream <br /> (function template)</DescriptionBody>
  </DescriptionItem>
</DescriptionList>
