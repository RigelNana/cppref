---
title: "std::disjunction"
description: "Forms the logical disjunction of the type traits B..., effectively performing a logical OR on the sequence of traits."
source_url: "https://en.cppreference.com/w/cpp/types/disjunction"
language: "C++"
---

{/* source:cpp-types-disjunction:0000 */}
<DeclarationDoc>
Defined in header [`<type_traits>`](/docs/cpp/library/headers/type_traits)

  <Declaration language="cpp" since="C++17" code={`template< class... B >
struct disjunction;`} />

  <DeclarationDescription>
    {/* source:cpp-types-disjunction:0001 */}
    Forms the [logical disjunction](https://en.wikipedia.org/wiki/Logical_disjunction) of the type traits `B...`, effectively performing a logical OR on the sequence of traits.

    {/* source:cpp-types-disjunction:0002 */}
    The specialization `std::disjunction<B1, ..., BN>` has a public and unambiguous base that is

    {/* source:cpp-types-disjunction:0003 */}
    - if `sizeof...(B) == 0`, `std::false_type`; otherwise the first type `Bi` in `B1, ..., BN` for which `bool(Bi::value) == true`, or `BN` if there is no such type.

    {/* source:cpp-types-disjunction:0004 */}
    The member names of the base class, other than `disjunction` and `operator=`, are not hidden and are unambiguously available in `disjunction`.

    {/* source:cpp-types-disjunction:0005 */}
    Disjunction is short-circuiting: if there is a template type argument `Bi` with `bool(Bi::value) != false`, then instantiating `disjunction<B1, ..., BN>::value` does not require the instantiation of `Bj::value` for `j > i`.

    {/* source:cpp-types-disjunction:0006 */}
    If the program adds specializations for `std::disjunction` or `std::disjunction_v`, the behavior is undefined.
  </DeclarationDescription>
</DeclarationDoc>

### Template parameters

{/* source:cpp-types-disjunction:0007 */}
<ParameterList>
  <Parameter name="B...">every template argument `Bi` for which `Bi::value` is instantiated must be usable as a base class and define member `value` that is convertible to `bool`</Parameter>
</ParameterList>

### Helper variable template

{/* source:cpp-types-disjunction:0008 */}
<DeclarationDoc>
  <Declaration language="cpp" since="C++17" code={`template< class... B >
constexpr bool disjunction_v = disjunction<B...>::value;`} />
  <DeclarationDescription>
  </DeclarationDescription>
</DeclarationDoc>

### Possible implementation

{/* source:cpp-types-disjunction:0009 */}
```cpp
template<class...>
struct disjunction : std::false_type {};
 
template<class B1>
struct disjunction<B1> : B1 {};
 
template<class B1, class... Bn>
struct disjunction<B1, Bn...>
    : std::conditional_t<bool(B1::value), B1, disjunction<Bn...>>  {};
```

### Notes

{/* source:cpp-types-disjunction:0010 */}
A specialization of `disjunction` does not necessarily inherit from of either `std::true_type` or `std::false_type`: it simply inherits from the first `B` whose `::value`, explicitly converted to `bool`, is `true`, or from the very last `B` when all of them convert to `false`. For example, `std::disjunction<std::integral_constant<int, 2>, std::integral_constant<int, 4>>::value` is `2`.

{/* source:cpp-types-disjunction:0011 */}
The short-circuit instantiation differentiates `disjunction` from [fold expressions](/docs/cpp/language/templates/fold): a fold expression like `(... || Bs::value)` instantiates every `B` in `Bs`, while `std::disjunction_v<Bs...>` stops instantiation once the value can be determined. This is particularly useful if the later type is expensive to instantiate or can cause a hard error when instantiated with the wrong type.

{/* source:cpp-types-disjunction:0012 */}
| [Feature-test](/docs/cpp/library/utility/feature_test) macro | Value | Std | Feature |
| --- | ---: | --- | --- |
| [`__cpp_lib_logical_traits`](/docs/cpp/library/feature_test#cpp_lib_logical_traits) | [`201510L`](/docs/cpp/library/compiler_support/17#cpp_lib_logical_traits_201510L) | (C++17) | [Logical operator type traits](/docs/cpp/library/meta#Operations_on_traits) |

### Example

{/* source:cpp-types-disjunction:0013 */}
```cpp
#include <cstdint>
#include <string>
#include <type_traits>
 
// values_equal<a, b, T>::value is true if and only if a == b.
template<auto V1, decltype(V1) V2, typename T>
struct values_equal : std::bool_constant<V1 == V2>
{
    using type = T;
};
 
// default_type<T>::value is always true
template<typename T>
struct default_type : std::true_type
{
    using type = T;
};
 
// Now we can use disjunction like a switch statement:
template<int I>
using int_of_size = typename std::disjunction< //
    values_equal<I, 1, std::int8_t>,           //
    values_equal<I, 2, std::int16_t>,          //
    values_equal<I, 4, std::int32_t>,          //
    values_equal<I, 8, std::int64_t>,          //
    default_type<void>                         // must be last!
    >::type;
 
static_assert(sizeof(int_of_size<1>) == 1);
static_assert(sizeof(int_of_size<2>) == 2);
static_assert(sizeof(int_of_size<4>) == 4);
static_assert(sizeof(int_of_size<8>) == 8);
static_assert(std::is_same_v<int_of_size<13>, void>);
 
// checking if Foo is constructible from double will cause a hard error
struct Foo
{
    template<class T>
    struct sfinae_unfriendly_check { static_assert(!std::is_same_v<T, double>); };
 
    template<class T>
    Foo(T, sfinae_unfriendly_check<T> = {});
};
 
template<class... Ts>
struct first_constructible
{
    template<class T, class...Args>
    struct is_constructible_x : std::is_constructible<T, Args...>
    {
        using type = T;
    };
 
    struct fallback
    {
        static constexpr bool value = true;
        using type = void; // type to return if nothing is found
    };
 
    template<class... Args>
    using with = typename std::disjunction<is_constructible_x<Ts, Args...>...,
                                           fallback>::type;
};
 
// OK, is_constructible<Foo, double> not instantiated
static_assert(std::is_same_v<first_constructible<std::string, int, Foo>::with<double>,
                             int>);
 
static_assert(std::is_same_v<first_constructible<std::string, int>::with<>, std::string>);
static_assert(std::is_same_v<first_constructible<std::string, int>::with<const char*>,
                             std::string>);
static_assert(std::is_same_v<first_constructible<std::string, int>::with<void*>, void>);
 
int main() {}
```

### See also

{/* source:cpp-types-disjunction:0014 */}
<DescriptionList>
  <DescriptionItem>
    <DescriptionTerm>[`negation`](/docs/cpp/library/meta/negation)<InlineRevision since="C++17" /></DescriptionTerm>
    <DescriptionBody>logical NOT metafunction <br /> (class template)</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>[`conjunction`](/docs/cpp/library/meta/conjunction)<InlineRevision since="C++17" /></DescriptionTerm>
    <DescriptionBody>variadic logical AND metafunction <br /> (class template)</DescriptionBody>
  </DescriptionItem>
</DescriptionList>
