...
virtual iter_type do_get( iter_type beg, iter_type end, std::ios_base& str,
  ...
  <DeclarationDescription>
    {/* source:cpp-locale-time-get-get:0001 */}
...
First, clears the error bits in `err` by executing `err = std::ios_base::goodbit`. Then enters a loop, which terminates whenever any of the following conditions becomes true (checked in this order):
    ...
    {/* source:cpp-locale-time-get-get:0008 */}
    ...
    {/* source:cpp-locale-time-get-get:0009 */}
    2) Parses one conversion specifier from the input sequence `[beg, end)` and updates the [`std::tm`](/docs/cpp/library/chrono/c/tm) structure pointed to by `t` accordingly.

    {/* source:cpp-locale-time-get-get:0010 */}
    First, clears the error bits in `err` by executing `err = std::ios_base::goodbit`. Then reads characters from the input sequence `[beg, end)` that are expected by the [`std::time_get`](/docs/cpp/library/text/locale/time_get) format specifier formed by combining `'%'`, `modifier` (if not `'\0'`), and `format`. If the characters do not combine to form a valid conversion specifier, sets `failbit` in `err`. If the end of the input stream is reached after reading a character, sets `eofbit` in `err`. If the input string was parsed successfully, updates the corresponding fields of `*t`.

    {/* source:cpp-locale-time-get-get:0011 */}
...
{/* source:cpp-locale-time-get-get:0012 */}
<ParameterList>
  ...
  <Parameter name="fmtbeg">pointer to the first character of a sequence of `char_type` characters specifying the conversion format (see below)</Parameter>
  <Parameter name="fmtend">pointer one past the last character of a sequence of `char_type` characters specifying the conversion format</Parameter>
  <Parameter name="format">the character that names a conversion specifier</Parameter>
  <Parameter name="modifier">the optional modifier that may appear between `%` and the conversion specifier</Parameter>
</ParameterList>

{/* source:cpp-locale-time-get-get:0013 */}
...
{/* source:cpp-locale-time-get-get:0014 */}
...
{/* source:cpp-locale-time-get-get:0015 */}
Note: `tm_isdst` is not written to, and needs to be set explicitly for use with functions such as `mktime`

### Return value

{/* source:cpp-locale-time-get-get:0016 */}
Iterator pointing one past the last character in `[beg, end)` that was parsed successfully.

### Notes

{/* source:cpp-locale-time-get-get:0017 */}
The case-insensitive comparison for the non-whitespace non-`'%'` characters in the format string, the [`std::collate`](/docs/cpp/library/text/locale/collate) facet of the locale provided by `str` is typically, but not necessarily, used.

{/* source:cpp-locale-time-get-get:0018 */}
If a parsing error is encountered, many implementations of this function leave `*t` completely untouched.

{/* source:cpp-locale-time-get-get:0019 */}
...
{/* source:cpp-locale-time-get-get:0020 */}
```cpp
#include <iomanip>
#include <iostream>
#include <locale>
...
{/* source:cpp-locale-time-get-get:0021 */}
<DescriptionList>
  <DescriptionItem>
