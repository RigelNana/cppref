...
description: "std::memory_order specifies how memory accesses, including regular, non-atomic memory accesses, are to be ordered around an atomic operation."
source_url: "https://en.cppreference.com/w/cpp/atomic/memory_order"
language: "C++"
---

{/* source:cpp-atomic-memory-order:0000 */}
<DeclarationDoc>
Defined in header [`<atomic>`](/docs/cpp/library/headers/atomic)

  <Declaration language="cpp" since="C++11" until="C++20" code={`enum memory_order
{
  ...
  <Declaration language="cpp" since="C++20" code={`enum class memory_order : /* unspecified */
{
...
{/* source:cpp-atomic-memory-order:0001 */}
`std::memory_order` specifies how memory accesses, including regular, non-atomic memory accesses, are to be ordered around an atomic operation. Absent any constraints on a multi-core system, when multiple threads simultaneously read and write to several variables, one thread can observe the values change in an order different from the order another thread wrote them. Indeed, the apparent order of changes can even differ among multiple reader threads. Some similar effects can occur even on uniprocessor systems due to compiler transformations allowed by the memory model.

{/* source:cpp-atomic-memory-order:0002 */}
The default behavior of all atomic operations in the library provides for sequentially consistent ordering (see discussion below). That default can hurt performance, but the library's atomic operations can be given an additional `std::memory_order` argument to specify the exact constraints, beyond atomicity, that the compiler and processor must enforce for that operation.

### Constants

{/* source:cpp-atomic-memory-order:0003 */}
Defined in header [`<atomic>`](/docs/cpp/library/headers/atomic)

<DescriptionList>
  <DescriptionItem>
    <DescriptionTerm>`memory_order_relaxed`</DescriptionTerm>
    <DescriptionBody>Relaxed operation: there are no synchronization or ordering constraints imposed on other reads or writes, only this operation's atomicity is guaranteed (see [Relaxed ordering](/docs/cpp/library/atomic/memory_order#Relaxed_ordering) below).</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>`memory_order_consume`</DescriptionTerm>
    <DescriptionBody>A load operation with this memory order performs a *consume operation* on the affected memory location: no reads or writes in the current thread dependent on the value currently loaded can be reordered before this load. Writes to data-dependent variables in other threads that release the same atomic variable are visible in the current thread. On most platforms, this affects compiler optimizations only (see [Release-Consume ordering](/docs/cpp/library/atomic/memory_order#Release-Consume_ordering) below).</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>`memory_order_acquire`</DescriptionTerm>
    <DescriptionBody>A load operation with this memory order performs the *acquire operation* on the affected memory location: no reads or writes in the current thread can be reordered before this load. All writes in other threads that release the same atomic variable are visible in the current thread (see [Release-Acquire ordering](/docs/cpp/library/atomic/memory_order#Release-Acquire_ordering) below).</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>`memory_order_release`</DescriptionTerm>
    <DescriptionBody>A store operation with this memory order performs the *release operation*: no reads or writes in the current thread can be reordered after this store. All writes in the current thread are visible in other threads that acquire the same atomic variable (see [Release-Acquire ordering](/docs/cpp/library/atomic/memory_order#Release-Acquire_ordering) below) and writes that carry a dependency into the atomic variable become visible in other threads that consume the same atomic (see [Release-Consume ordering](/docs/cpp/library/atomic/memory_order#Release-Consume_ordering) below).</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>`memory_order_acq_rel`</DescriptionTerm>
    <DescriptionBody>A read-modify-write operation with this memory order is both an *acquire operation* and a *release operation*. No memory reads or writes in the current thread can be reordered before the load, nor after the store. All writes in other threads that release the same atomic variable are visible before the modification and the modification is visible in other threads that acquire the same atomic variable.</DescriptionBody>
  </DescriptionItem>
  <DescriptionItem>
    <DescriptionTerm>`memory_order_seq_cst`</DescriptionTerm>
    <DescriptionBody>A load operation with this memory order performs an  *acquire operation*, a store performs a *release operation*, and read-modify-write performs both an *acquire operation* and a *release operation*, plus a single total order exists in which all threads observe all modifications in the same order (see [Sequentially-consistent ordering](/docs/cpp/library/atomic/memory_order#Sequentially-consistent_ordering) below).</DescriptionBody>
  </DescriptionItem>
</DescriptionList>

### Formal description

{/* source:cpp-atomic-memory-order:0004 */}
...
{/* source:cpp-atomic-memory-order:0005 */}
Within the same thread, evaluation A may be *sequenced-before* evaluation B, as described in [evaluation order](/docs/cpp/language/expressions/eval_order).

#### Carries dependency

{/* source:cpp-atomic-memory-order:0006 */}
...
{/* source:cpp-atomic-memory-order:0007 */}
1. The value of A is used as an operand of B, except
   {/* source:cpp-atomic-memory-order:0008 */}
   a) if B is a call to [`std::kill_dependency`](/docs/cpp/library/atomic/kill_dependency),
   {/* source:cpp-atomic-memory-order:0009 */}
   b) if A is the left operand of the built-in `&&`, `||`, `?:`, or `,` operators.
{/* source:cpp-atomic-memory-order:0010 */}
2. A writes to a scalar object M, B reads from M.
{/* source:cpp-atomic-memory-order:0011 */}
3. A carries dependency into another evaluation X, and X carries dependency into B.

#### Modification order

{/* source:cpp-atomic-memory-order:0012 */}
All modifications to any particular atomic variable occur in a total order that is specific to this one atomic variable.

{/* source:cpp-atomic-memory-order:0013 */}
The following four requirements are guaranteed for all atomic operations:

{/* source:cpp-atomic-memory-order:0014 */}
...
{/* source:cpp-atomic-memory-order:0015 */}
...
{/* source:cpp-atomic-memory-order:0016 */}
...
{/* source:cpp-atomic-memory-order:0017 */}
...
{/* source:cpp-atomic-memory-order:0018 */}
...
{/* source:cpp-atomic-memory-order:0023 */}
Also, some library calls may be defined to synchronize-with other library calls on other threads.

#### Dependency-ordered before

{/* source:cpp-atomic-memory-order:0024 */}
Between threads, evaluation A is dependency-ordered before evaluation B if any of the following is true:

{/* source:cpp-atomic-memory-order:0025 */}
1) A performs a *release operation* on some atomic M, and, in a different thread, B performs a *consume operation* on the same atomic M, and B reads a value written <InlineRevision until="C++20">by any part of the release sequence headed</InlineRevision> by A.

{/* source:cpp-atomic-memory-order:0026 */}
2) A is dependency-ordered before X and X carries a dependency into B.

#### Inter-thread happens-before

{/* source:cpp-atomic-memory-order:0027 */}
Between threads, evaluation A inter-thread happens before evaluation B if any of the following is true:

{/* source:cpp-atomic-memory-order:0028 */}
1) A *synchronizes-with* B.

{/* source:cpp-atomic-memory-order:0029 */}
2) A is dependency-ordered before B.

{/* source:cpp-atomic-memory-order:0030 */}
3) A *synchronizes-with* some evaluation X, and X is *sequenced-before* B.

{/* source:cpp-atomic-memory-order:0031 */}
4) A is *sequenced-before* some evaluation X, and X inter-thread happens-before B.

{/* source:cpp-atomic-memory-order:0032 */}
5) A inter-thread happens-before some evaluation X, and X inter-thread happens-before B.

#### Happens-before

{/* source:cpp-atomic-memory-order:0033 */}
Regardless of threads, evaluation A *happens-before* evaluation B if any of the following is true:

{/* source:cpp-atomic-memory-order:0034 */}
1) A is *sequenced-before* B.

{/* source:cpp-atomic-memory-order:0035 */}
2) A inter-thread happens before B.

{/* source:cpp-atomic-memory-order:0036 */}
The implementation is required to ensure that the *happens-before* relation is acyclic, by introducing additional synchronization if necessary (it can only be necessary if a consume operation is involved, see [Batty et al](https://www.cl.cam.ac.uk/~pes20/cpp/popl085ap-sewell.pdf)).

{/* source:cpp-atomic-memory-order:0037 */}
If one evaluation modifies a memory location, and the other reads or modifies the same memory location, and if at least one of the evaluations is not an atomic operation, the behavior of the program is undefined (the program has a [data race](/docs/cpp/language/basic_concepts/memory_model)) unless there exists a *happens-before* relationship between these two evaluations.

{/* source:cpp-atomic-memory-order:0038 */}
<Revision since="C++20">
...
{/* source:cpp-atomic-memory-order:0041 */}
...
{/* source:cpp-atomic-memory-order:0042 */}
1) A *happens-before* B.

{/* source:cpp-atomic-memory-order:0043 */}
2) There is no other side effect X to M where A happens-before X and X happens-before B.

{/* source:cpp-atomic-memory-order:0044 */}
...
{/* source:cpp-atomic-memory-order:0045 */}
...
{/* source:cpp-atomic-memory-order:0046 */}
Atomic load with `memory_order_consume` or stronger is a consume operation. Note that [`std::atomic_thread_fence`](/docs/cpp/library/atomic/atomic_thread_fence) imposes stronger synchronization requirements than a consume operation.

#### Acquire operation

{/* source:cpp-atomic-memory-order:0047 */}
Atomic load with `memory_order_acquire` or stronger is an acquire operation. The `lock()` operation on a [Mutex](/docs/cpp/library/named_req/Mutex) is also an acquire operation. Note that [`std::atomic_thread_fence`](/docs/cpp/library/atomic/atomic_thread_fence) imposes stronger synchronization requirements than an acquire operation.

#### Release operation

{/* source:cpp-atomic-memory-order:0048 */}
Atomic store with `memory_order_release` or stronger is a release operation. The `unlock()` operation on a [Mutex](/docs/cpp/library/named_req/Mutex) is also a release operation. Note that [`std::atomic_thread_fence`](/docs/cpp/library/atomic/atomic_thread_fence) imposes stronger synchronization requirements than a release operation.

### Explanation

#### Relaxed ordering

{/* source:cpp-atomic-memory-order:0049 */}
Atomic operations tagged `memory_order_relaxed` are not synchronization operations; they do not impose an order among concurrent memory accesses. They only guarantee atomicity and modification order consistency.

{/* source:cpp-atomic-memory-order:0050 */}
For example, with `x` and `y` initially zero,

{/* source:cpp-atomic-memory-order:0051 */}
```cpp
// Thread 1:
...
{/* source:cpp-atomic-memory-order:0052 */}
...
{/* source:cpp-atomic-memory-order:0053 */}
<Revision since="C++14">
...
{/* source:cpp-atomic-memory-order:0054 */}
Typical use for relaxed memory ordering is incrementing counters, such as the reference counters of [`std::shared_ptr`](/docs/cpp/library/memory/shared_ptr), since this only requires atomicity, but not ordering or synchronization (note that decrementing the `std::shared_ptr` counters requires acquire-release synchronization with the destructor).

{/* source:cpp-atomic-memory-order:0055 */}
```cpp
#include <atomic>
#include <iostream>
#include <thread>
...
{/* source:cpp-atomic-memory-order:0056 */}
...
{/* source:cpp-atomic-memory-order:0057 */}
...
{/* source:cpp-atomic-memory-order:0058 */}
The synchronization is established only between the threads releasing and acquiring the same atomic variable. Other threads can see different order of memory accesses than either or both of the synchronized threads.

{/* source:cpp-atomic-memory-order:0059 */}
...
{/* source:cpp-atomic-memory-order:0060 */}
Mutual exclusion locks, such as [`std::mutex`](/docs/cpp/library/thread/mutex) or [atomic spinlock](/docs/cpp/library/atomic/atomic_flag), are an example of release-acquire synchronization: when the lock is released by thread A and acquired by thread B, everything that took place in the critical section (before the release) in the context of thread A has to be visible to thread B (after the acquire) which is executing the same critical section.

{/* source:cpp-atomic-memory-order:0061 */}
```cpp
#include <atomic>
#include <cassert>
#include <string>
...
{/* source:cpp-atomic-memory-order:0062 */}
...
{/* source:cpp-atomic-memory-order:0063 */}
...
{/* source:cpp-atomic-memory-order:0064 */}
...
{/* source:cpp-atomic-memory-order:0065 */}
...
{/* source:cpp-atomic-memory-order:0068 */}
See also [`std::kill_dependency`](/docs/cpp/library/atomic/kill_dependency) and [[`carries_dependency`](/docs/cpp/language/declarations/attributes/carries_dependency)] for fine-grained dependency chain control.

{/* source:cpp-atomic-memory-order:0069 */}
...
{/* source:cpp-atomic-memory-order:0070 */}
<Revision since="C++17">
...
{/* source:cpp-atomic-memory-order:0071 */}
...
{/* source:cpp-atomic-memory-order:0072 */}
Atomic operations tagged `memory_order_seq_cst` not only order memory the same way as release/acquire ordering (everything that happened-before a store in one thread becomes a visible side effect in the thread that did a load), but also establish a single total modification order of all atomic operations that are so tagged.

{/* source:cpp-atomic-memory-order:0073 */}
<Revision until="C++20">
Formally,
...
If there was a `memory_order_seq_cst` [`std::atomic_thread_fence`](/docs/cpp/library/atomic/atomic_thread_fence) operation X *sequenced-before* B, then B observes one of the following:
...
For a pair of atomic operations on M called A and B, where A writes and B reads M's value, if there are two `memory_order_seq_cst` [`std::atomic_thread_fences`](/docs/cpp/library/atomic/atomic_thread_fence) X and Y, and if A is *sequenced-before* X, Y is *sequenced-before* B, and X appears before Y in the Single Total Order, then B observes either:
...
- there is a `memory_order_seq_cst` [`std::atomic_thread_fence`](/docs/cpp/library/atomic/atomic_thread_fence) X such that A is *sequenced-before* X and X appears before B in the Single Total Order,
- or, there is a `memory_order_seq_cst` [`std::atomic_thread_fence`](/docs/cpp/library/atomic/atomic_thread_fence) Y such that Y is *sequenced-before* B and A appears before Y in the Single Total Order,
- or, there are `memory_order_seq_cst` [`std::atomic_thread_fences`](/docs/cpp/library/atomic/atomic_thread_fence) X and Y such that A is *sequenced-before* X, Y is *sequenced-before* B, and X appears before Y in the Single Total Order.
...
is allowed to produce `r1 == 1 && r2 == 3 && r3 == 0`, where A *happens-before* C, but C precedes A in the single total order C-E-F-A of `memory_order_seq_cst` (see [Lahav et al](https://plv.mpi-sws.org/scfix/paper.pdf)).
...
{/* source:cpp-atomic-memory-order:0074 */}
...
{/* source:cpp-atomic-memory-order:0075 */}
...
{/* source:cpp-atomic-memory-order:0076 */}
...
{/* source:cpp-atomic-memory-order:0077 */}
Within a thread of execution, accesses (reads and writes) through [volatile glvalues](/docs/cpp/language/declarations/cv) cannot be reordered past observable side-effects (including other volatile accesses) that are sequenced-before or sequenced-after within the same thread, but this order is not guaranteed to be observed by another thread, since volatile access does not establish inter-thread synchronization.

{/* source:cpp-atomic-memory-order:0078 */}
In addition, volatile accesses are not atomic (concurrent read and write is a [data race](/docs/cpp/language/basic_concepts/memory_model)) and do not order memory (non-volatile memory accesses may be freely reordered around the volatile access).

{/* source:cpp-atomic-memory-order:0079 */}
One notable exception is Visual Studio, where, with default settings, every `volatile` write has release semantics and every `volatile` read has acquire semantics ([Microsoft Docs](https://learn.microsoft.com/en-us/cpp/cpp/volatile-cpp)), and thus volatiles may be used for inter-thread synchronization. Standard volatile semantics are not applicable to multi-threaded programming, although they are sufficient for e.g. communication with a [`std::signal`](/docs/cpp/library/utility/program/signal) handler that runs in the same thread when applied to `sig_atomic_t` variables.

### See also

{/* source:cpp-atomic-memory-order:0080 */}
<DescriptionList>
  <DescriptionItem>
    <DescriptionTerm>[C documentation](/docs/c/library/thread/memory_order) for `memory order`</DescriptionTerm>
...
{/* source:cpp-atomic-memory-order:0081 */}
1. [MOESI protocol](https://en.wikipedia.org/wiki/MOESI_protocol)
...
{/* source:cpp-atomic-memory-order:0082 */}
